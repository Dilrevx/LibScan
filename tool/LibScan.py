...
def parse_arguments():
    ...
    parser_one.add_argument(
        "-af", metavar="FOLDER", type=str, help="Specify directory of apps"
    )
    parser_one.add_argument(
        "-lf", metavar="FOLDER", type=str, help="Specify directory of TPL versions"
    )
    parser_one.add_argument(
        "-ld",
        metavar="FOLDER",
...
def jar_to_dex(libs_folder, lib_dex_folder):
    ...
    for file in os.listdir(libs_folder):
        ...
        classpath = "/home/li/LibScan/tmp/dex-tools-v2.4/lib/dx-30.0.2.jar"

        cmd = f"java -Xms512m -Xmx2048m -classpath {classpath} com.android.dx.command.Main --dex --min-sdk-version 36  --output={lib_dex_folder + '/' + file_name + '.dex'} {libs_folder + '/' + file} "
        ...
        if file.endswith(".zip"):
            ...
            for f in os.listdir(libs_folder):
                if f == "classes.jar":
                    os.rename(
                        libs_folder + "/" + f,
                        libs_folder + "/" + file[: file.rfind(".")] + ".jar",
                    )
            zip_file.close()
...
def main(
    ...
    """
    1. 将库目录下所有的arr、jar文件转化为dex文件，并放入libs_dex目录下
    2. 在库级别并行分析或在apk级别并行分析
    """
...
if __name__ == "__main__":
    ...
    os.makedirs(args.ld, exist_ok=True)
